...
const ChangeContact = ({allProducts, setAllProducts, currentElem}) => {
    ...
    const [margent, setMargent] = useState('')
    const [IVA, setIVA] = useState('')

    const [marketPriceDP, setMarketPriceDP] = useState('')
    const [entryPriceDP, setEntryPriceDP] = useState('')
    const [profitDP, setProfitDP] = useState(0)
    const [margentDP, setMargentDP] = useState('')
    const [IVADP, setIVADP] = useState('')

    const [marketPriceSDP, setMarketPriceSDP] = useState('')
    const [entryPriceSDP, setEntryPriceSDP] = useState('')
    const [profitSDP, setProfitSDP] = useState(0)
    const [margentSDP, setMargentSDP] = useState('')
    const [IVASDP, setIVASDP] = useState('')


    const [marketPriceCP, setMarketPriceCP] = useState('')
    const [entryPriceCP, setEntryPriceCP] = useState('')
    const [profitCP, setProfitCP] = useState(0)
    const [margentCP, setMargentCP] = useState('')
    const [IVACP, setIVACP] = useState('')


    const [tags, setTags] = useState(([]).join())
    ...
    const updateProd = () => {
        ...
        // formData.append('customFile', file);
        // formData.append('informData', JSON.stringify(informData))
        axios.post('https://tpomobi.shop/updateContactAdmin', {data: informData, _id: currentElem._id}).then(async function(response) {
                        ...
                        <div className='addProductLeftTopRight'>
                            ...
                            <div className='addProductLeftTopRightInfo'>
                                <div className='skuBlock'>
                                    ...
                                    <Input disabled={true} style={{cursor: 'pointer', height: '28px', maxWidth: '379.63px', margin: '5px 0px'}} defaultValue={"ID's places"} placeholder='Codigo' type="text"/>
            ...
            </div>
            {
                ...
                <div className='modalAdmin' onClick={() => setOpenModal(false)}>
                    <div className='profitModalAdmin' onClick={(e) => e.stopPropagation()}>
                        <div className='seeProfitText'>Add <CgClose className="closeButton" onClick={() => setOpenModal(false)}/></div>
