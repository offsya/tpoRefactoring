...
import {addallContacts,updateContact} from "../../../../features/allContacts";
...
const ChangeContactTwo = ({allProducts, setAllProducts, currentElem}) => {

    const [currentFileImage, setCurrentFileImage] = useState(currentElem.img)
    const dispatch = useDispatch();
    const [timer, setTimer] = useState(currentElem?.time || 0)
    ...
    const [solvencia, setSolvencia] = useState(currentElem.solvencia)
    const [dificilidad, setDificilidad] = useState(currentElem.dificilidad)
    const [extraPedidos, setExtraPedidos] = useState(currentElem.extraPedidos)
    ...
    const [tipo, setTipo] = useState(currentElem?.tipo || '')
    const [group, setGroup] = useState(currentElem?.group || '')
    ...
    const [username, setUserName] = useState(currentElem.username || '')
    const [employee, setEmployee] = useState(currentElem.employee || '')
    const [responsable, setResponsable] = useState(currentElem.responsable || '')

    const activeAcc = useMemo(() => {
        let buff = allContacts.filter(acc => acc.type != 'userUnion' && (acc.phone == currentElem.phone))
        return buff.map(el => el?.platform ? el?.platform : 'browser')
    }, [])
    ...
    const [tags, setTags] = useState(currentElem.tags)
    const [IBAN, setIBAN] = useState(currentElem.iban)
    const [SWIFT, setSWIFT] = useState(currentElem.swift)
    const [nif, setNIF] = useState(currentElem.nif)
    const [cif, setCIF] = useState(currentElem.cif)
    ...
    const copyProd = () => {
        ...
        axios.post('https://localhost:2222/createContactAdmin', {data: informData}).then(function (response) {
            dispatch(setValue(2))
            dispatch(({...informData, "_id": response.data.insertedId}))
    ...
    const [checkboxContact,setCheckboxContact ] = useState(currentElem?.type ? (currentElem?.type != 'client' ? 1 : 2) : 2)
    ...
    const [rateValue, setRateValue] = useState(currentElem.rate)
    ...
    const currentClient = useMemo(() => {
        let buff = allContacts.filter(el => {
            if (el.type == 'userUnion' && el.tradeName == currentElem.tradeName) {
                return true
            }
    ...
    }, [allClientOrders, sortQuantityOrPrice])
    ...
    return (
        <div className='tableMainClass'>
            ...
            <div className='addContactContainer'>
                            ...
                            <div className="addContactContainerTopElem2Left" style={{background:checkboxContact==1 ? "#B4B8DA":"#AAEAF8"}}>
                                ...
                                <div className="textElem">
                                    ...
                                    <Select
                                        ...
                                        defaultValue={() => {
                                            let bf = [];
                                            allContactTradesNameHandler.forEach((el, index) => el.label == currentElem.tradeName && bf.push(el.label))
                                            return bf
                                        }}
                                    />
                                    {/*<Select*/}
                ...
                <div className='addContactContainerRight'>
                    <div  className='addContactContainerRightUpperSwitcher'>
                        <div className="addContactContainerRightUpperSwitcherValue" style={{width: currentElem.type == 'client' ? '180px' : '100%', background: clientInfoSwitcher == 1 && 'white', color: clientInfoSwitcher == 1 && 'black'}} onClick={() => setClientInfoSwitcher(2)}>Top ordered products</div>
                        {currentElem.type == 'client' && <div className="addContactContainerRightUpperSwitcherValue" style={{width: '122px', background: clientInfoSwitcher == 2 && 'white', color: clientInfoSwitcher == 2 && 'black'}} onClick={() => setClientInfoSwitcher(1)}>Users</div>}
                    </div>
                    ...
                    {clientInfoSwitcher == 1 &&
                        <div className='addContactContainerRightStats' style={{display: 'block', maxHeight: '400px', marginTop: '5px'}}>
                            <div style={{display: 'flex', justifyContent: 'flex-start', alignItems: 'center', flexDirection: 'column', height: '418px', overflow: 'scroll'}}>
                                {/*{currentElem?.type == 'userUnion' && currentUsersUnion.map((el, index) => {*/}
                                {/*    return (*/}
                                {/*        <div>*/}
                                ...
                                {currentElem?.type == 'client' && currentUsersUnion.map((el, index) => {
                                    return (
                                        <div>
                                            <div className='usersElem' style={{background: openUserInfo.includes(index) && '#F1F1F1', height: openUserInfo.includes(index) && '200px', borderBottom: openUserInfo.includes(index) && 'none', borderRadius: openUserInfo.includes(index) && '7px', marginBottom: openUserInfo.includes(index) && '5px', marginTop: openUserInfo.includes(index) && '5px', width: '319px'}}>
                                                <div className='userElemCard'>
            ...
            <OrdersComponentContact updateProd={updateProd} currentUser={currentElem}/>

        </div>
    );
};

export default ChangeContactTwo;