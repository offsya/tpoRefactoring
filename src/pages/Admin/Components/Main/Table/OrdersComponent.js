...
const OrdersComponent = ({}) => {
    ...
    const currentTable = () => {
        ...
        if(addProduct == 2){return <OrderContainer allProducts={allProducts} setSelectedElems={setSelectedElems} setCurrentElem={setCurrentElem}/>}
                    ...
                    <div className='selectedButtons'>
                        ...
                        {addProduct == 2 ?
                            <div className='selectButton'>
                                ...
                                <span style={{color: "#00AB55"}}>{((allOrdersPrices.map(elem => elem.marketPriceCP ? (parseFloat(elem.marketPriceCP*(elem.quantity || 1)) - parseFloat(elem.ourPrice*(elem.quantity || 1))) *  (1-parseFloat(elem.IVA)/100) : 0)).reduce((acc, num) => acc + num, 0)).toFixed(2)}€</span>
                            </div>
                            :
                            <div className='selectButton'>
                                <div style={{textAlign: 'center', fontWeight: '400'}}>Estimated Profit</div>
                                <span style={{color: "#00AB55"}}>{((allOrdersProductsPrices.map(elem => elem.marketPriceCP ? (parseFloat(elem.marketPriceCP*(elem.quantity || 1)) - parseFloat(elem.ourPrice*(elem.quantity || 1))) *  (1-parseFloat(elem.IVA)/100) : 0)).reduce((acc, num) => acc + num, 0)).toFixed(2)}€</span>
                            </div>
