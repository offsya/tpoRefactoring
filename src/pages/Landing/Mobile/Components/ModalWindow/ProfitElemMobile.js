...
const ProfitElemMobile = ({elem}) => {
    ...
    return (
        <div className='profitTableElem profitTableElemBorder'>
            <span className="profitProductName-mobile"><img className='imgCardElemProfit-mobile' src={elem.img} alt="tomato"/>{elem.name}</span>
            <span className="profitProductMiddle profitSize-mobile">{parseFloat(parseFloat(elem.marketPriceCP).toFixed(2)) != 'NaN' ? parseFloat(parseFloat(elem.marketPriceCP).toFixed(2)) : 0}€</span>
            <span className='profitOurPrice profitSize-mobile'>{(elem.ourPriceCP * (chooseOption == 1 ? elem.CP : elem.DP) / 100 + elem.ourPriceCP).toFixed(2)}€</span>
            <div className='cardElemChange cardElemChange-mobile'><FiMinus onClick={() =>
            {
            ...
            <span className='profitProfit profitSize-mobile'>{((elem.marketPriceCP - (elem.ourPriceCP * (chooseOption == 1 ? elem.CP : elem.DP) / 100 + elem.ourPriceCP)) * elem.quantity).toFixed(2)}€</span>
        </div>
    );
};

export default ProfitElemMobile;